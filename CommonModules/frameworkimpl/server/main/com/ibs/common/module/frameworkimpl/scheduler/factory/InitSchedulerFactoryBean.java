package com.ibs.common.module.frameworkimpl.scheduler.factory;
...
public class InitSchedulerFactoryBean extends SchedulerFactoryBean {
	...
	protected void startScheduler(Scheduler arg0, int arg1)
		...
		for (JobDetail jobDetail : jobDetails) {
			...
			List<SchedulerSetting> schedulerSettings = schedulerSettingDao.findSchedulerSettingByName(jobDetail.getName());
...
//		}
		...
		try {
			...
			addOrUpdateTrigger(cronTrigger.getName(), cronTrigger.getGroup(), cronTrigger);
			
//			Date theDate = this.getScheduler().rescheduleJob(cronTrigger.getName(), cronTrigger.getGroup(), cronTrigger);
//
//			//如果该触发器已经存在，则首先添加该触发器，然后再做rescheduleJob
//			if(theDate==null){
//				this.getScheduler().scheduleJob(cronTrigger);
//				this.getScheduler().rescheduleJob(cronTrigger.getName(), cronTrigger.getGroup(), cronTrigger);
//			}
		} catch (Exception e) {
	...
	public void schedule(Map<String, Object> map, JobDetail jobDetail) {
		...
		if (tempDate != null) {
			if (tempDate.getTime() < now.getTime()) {
		        //long ts = TriggerUtils.getNextGivenMinuteDate(null, 1).getTime();
				// 一分钟以后启动job
		...
		//设置执行次数
		Integer tempInt = (Integer)map.get(InitSchedulerFactoryBean.REPEATCOUNT);
		...
		try {
			...
			addOrUpdateTrigger(simpleTrigger.getName(), simpleTrigger.getGroup(), simpleTrigger);
//			Date theDate = this.getScheduler().rescheduleJob(simpleTrigger.getName(), simpleTrigger.getGroup(), simpleTrigger);
//			
//			//如果该触发器已经存在，则首先添加该触发器，然后再做rescheduleJob
//			if(theDate==null){
//				this.getScheduler().scheduleJob(simpleTrigger);
//				this.getScheduler().rescheduleJob(simpleTrigger.getName(), simpleTrigger.getGroup(), simpleTrigger);
//			}			
	...
	private boolean removeTrigger(String triggerName, String group) {
		try {
			this.getScheduler().pauseTrigger(triggerName, group);// 停止触发器
			boolean result = this.getScheduler().unscheduleJob(triggerName, group);// 移除触发器
			return result;
		} catch (SchedulerException e) {
			throw new RuntimeException(e);
		}
	...
	private void startJob(SchedulerSetting setting, JobDetail jobDetail) {
		logger.debug("start job: " + setting.getSchedulerId());
		
//		String jobGroup = Scheduler.DEFAULT_GROUP;
//		if (StringUtils.isNotEmpty(setting.getSchedulerWar()))
//			jobGroup = setting.getSchedulerWar();
		...
		if (setting.isEnable()) {
			...
			if (setting.isCronType()) {
				jobDetail.getJobDataMap().put(SCHEDULERID, setting.getSchedulerId());
	...
	public void removeJob(String schedulerId) throws SchedulerException {
		JobDetail jobDetail = getRunningJobDetail(schedulerId);
		this.getScheduler().deleteJob(jobDetail.getName(), jobDetail.getGroup());
	}
	...
	public int getJobState(String schedulerId) throws SchedulerException {
		...
		if (jobDetail == null)
			throw new RuntimeException("no job detail");
		return this.getScheduler().getTriggerState(jobDetail.getName(), jobDetail.getGroup());
	}
