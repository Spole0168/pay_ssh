	...
	$.jstree.plugin("checkbox2", {
		...
		_fn : {
			_prepare_checkboxes : function (obj) {
				obj = !obj || obj == -1 ? this.get_container() : this._get_node(obj);
				var c, _this = this, t;
				var settings = this._get_settings().checkbox2;
				obj.each(function () {
					t = $(this);
					if(settings.associate_children)
						c = t.is("li") && t.hasClass("jstree-checked") ? "jstree-checked" : "jstree-unchecked";
					else
						c = "jstree-unchecked";
					t.find("a").not(":has(.jstree-checkbox)").prepend("<ins class='jstree-checkbox'>&#160;</ins>").parent().not(".jstree-checked, .jstree-unchecked").addClass(c);
			...
			change_state : function (obj, state) {
				var settings = this.get_settings().checkbox2;
				obj = this._get_node(obj);
				if(obj === false){
					return;
				}
				...
				if(state) {
					...
					} else if(obj.children("ul").children(".jstree-checked, .jstree-undetermined").length) {
						obj.removeClass("jstree-checked jstree-undetermined").addClass("jstree-undetermined");
					}
					else {
				...
				obj.parentsUntil(".jstree", "li").each(function () {
					var $this = $(this);
					if(!settings.associate_parent && $this.hasClass("jstree-checked")) {
						return false;
					}
					if(state) {
						if(settings.associate_parent) {
							if($this.children("ul").children(".jstree-checked, .jstree-undetermined").length) {
								$this.parentsUntil(".jstree", "li").andSelf().removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
								return false;
							}
							...
							if($this.children("ul").children(".jstree-checked, .jstree-undetermined").length) {
								...
								$this.removeClass("jstree-undetermined").addClass("jstree-unchecked");
							}
						}
					}
					else {
			...
			get_checked : function (obj) {
				obj = !obj || obj === -1 ? this.get_container() : this._get_node(obj);
				return obj.find("> ul > .jstree-checked, .jstree-checked > ul > .jstree-checked, .jstree-undetermined > ul > .jstree-checked");
			},
			get_unchecked : function (obj) { 
				obj = !obj || obj === -1 ? this.get_container() : this._get_node(obj);
				return obj.find("> ul > .jstree-unchecked, .jstree-undetermined > ul > .jstree-unchecked");
			...
			_repair_state : function (obj) {
				var settings = this.get_settings().checkbox2;
				if(!settings.associate_parent || !settings.associate_children) return;
				obj = this._get_node(obj);
				if(!obj.length || (!settings.associate_parent && obj.hasClass("jstree-checked"))) { return; }
				var a = obj.find("> ul > .jstree-checked").length,
					b = obj.find("> ul > .jstree-undetermined").length,
					c = obj.find("> ul > li").length;
				...
				if(c === 0) {
					if(obj.hasClass("jstree-undetermined")) {
						settings.associate_parent ? this.check_node(obj) : this.uncheck_node(obj);
					}
				}
				else if(a === 0 && b === 0) { this.uncheck_node(obj); }
				else if(a === c && settings.associate_parent) { this.check_node(obj); }
				else {
					obj.parentsUntil(".jstree","li").removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
				}
			},
		...
		_fn : {
			search : function (str, skip_async) {
				...
				if(!skip_async && s.ajax !== false && this.get_container().find(".jstree-closed:eq(0)").length > 0) {
					...
					if(!s.ajax.data) { s.ajax.data = { "search_string" : encodeURI(str) }; }
